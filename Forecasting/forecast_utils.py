...
def VAR_model(train_data, forecast_data):

    errors = {'nrmse':None, 'mape':None}
    data = {'Target':forecast_data, 'Predictions':None}
    ...
    results = model_fitted.forecast(train_data.values[-lag_order:], forecasting_steps)
    
    data['Predictions'] = pd.DataFrame(data=results, columns=forecast_data.columns, index=forecast_data.index)
...
class MLP_Forecasting:
    ...
    def split_sequence(self, data, train_test=False, scale=False):
        ...
        """
        Docstring:
        This function transforms the raw dataset(TS) into input(Features) and output.(Supervised)
        ...
        Attributes
        ----------
    ...
    def build_dense_layers(self, n_steps_in, n_steps_out, units, scale=False, epochs=100, lr=0.03, batch=256, verbose=1):
        ...
        n_steps_out: int 
            number of multi-forecasting points
        units: int
            number of NN units
        scale: bool
        ...
        batch: int 
            number of training epochs
        lr: float
            optimizer's leaarninig rate
        verbose: bool, optional
            to show the training steps
        
        Returns
        ----------
        A trained model on the data.
        """
    ...
    def forecast(self, test):
        ...
        errors['nrmse'] = nrmse(results)
        errors['mape'] = mape(results)
...
def plot_errors(Results:dict, weeks_days:str, models:list, grids:list=['Grid(52,56)']):
    
    if not grids:
        grids = list(Results.keys())
    if not models:
        models = list(Results[grids[0]].keys())
    n_weeks = len(Results[grids[0]][models[0]][weeks_days])
    n_days = len(Results[grids[0]][models[0]][weeks_days])

    length = n_days if 'Day' in weeks_days else n_weeks
    
    for grid in grids:
        print('='*40,grid,'='*40)
        fig, axes = plt.subplots(1, len(models), figsize=(20,5))
        for i, model in enumerate(models):
            errors = pd.DataFrame(index=['%s %s'%(weeks_days,i) for i in range(1,length+1,1)])
            errors['nrmse'] = [Results[grid]['%s'%model]['%s'%weeks_days]['%d'%(i+1)]['errors']['nrmse'] for i in range(length)]
            errors['mape'] = [Results[grid]['%s'%model]['%s'%weeks_days]['%d'%(i+1)]['errors']['mape'] for i in range(length)]
            ax = errors.plot(kind='bar', figsize=(20,8), rot='45', ax=[axes if len(models)==1 else axes[i]][0])
            ax.set_title('%s with %s'%(model, grid))
        plt.show()
        
def errors_box(Results, grid, error):
