...
nrmse = SourceFileLoader("nrmse", "./impute_utils.py").load_module().nrmse
...
def rmse_loss(ori_data, imputed_data, data_m):
    ...
    # Only computes for missing values
...
def show_results(data_x, miss_data_x, data_m, imputed_data, num_examples=1, cmap=None):
    for i in range(num_examples):
        ...
        plt.subplot(1,4,1)
        plt.title('Original')
        plt.imshow(data_x[i], cmap=cmap)
        plt.subplot(1,4,2)
        plt.title('Original With NaN')
        plt.imshow(miss_data_x[i], cmap=cmap)
        plt.subplot(1,4,3)
        plt.title('NaN Mask')
        plt.imshow(data_m[i], cmap=cmap)
        plt.subplot(1,4,4)
        plt.title('Imputed')
        plt.imshow(imputed_data[i], cmap=cmap);
        plt.show()